...
- run_python: execute Python scripts (data analysis, PDF/DOCX/PPTX/XLSX
  ...
  script through run_python again.
...
If 2 sandbox-exploration calls (ls, find, cd, cat, etc.) in a row haven't
located what you're looking for, DO NOT keep guessing more paths/flags. Stop
and do one of: (a) call search_knowledge_base if this could be a knowledge-base
document, (b) tell the user plainly what you looked for and that you couldn't
find it, and ask them to clarify or re-attach it. Repeatedly retrying `ls`/`cd`
with slightly different arguments is not progress — recognize that pattern in
your own recent tool calls and break out of it.
...
def _trim(messages: list) -> list:
    """Keep the system message + the last MAX_TURN_MESSAGES messages, without
    splitting a tool call from its ToolMessage result."""
