...
**Summary:** The image contains two histograms: the first, titled "Raw Price (Power Law)," displays a skewed distribution; the second, titled "Log Price (Normal)," shows a roughly normal distribution. Both histograms have "price" on the x-axis and "Count" on the y-axis.

![](https://res.cloudinary.com/douppa0ns/image/upload/v1768385779/chatbot/files/img_p4_1_lg0up7.png)
...
**Summary:** The image is a correlation matrix visualization, presented as a heatmap. The heatmap uses colors to represent correlation values between various features, with a color bar indicating the value range.

![](https://res.cloudinary.com/douppa0ns/image/upload/v1768385783/chatbot/files/img_p5_1_d01hhh.png)
...
**Summary:** The image contains three graphs showing relationships between the price of something and its characteristics. The graphs show the relationship between price and living space, construction grade, and view rating.

![](https://res.cloudinary.com/douppa0ns/image/upload/v1768385787/chatbot/files/img_p6_1_fwk7vl.png)


### Figure 3: Key Feature Relationships

**Living Space vs Price**

| sqft\_living ($10^n$) | Price ($10^n$ range)      |
| --------------------- | ------------------------- |
| $10^3$                | $10^5$ to $5 \times 10^5$ |
| $10^4$                | $10^6$ to $5 \times 10^6$ |
...
**Summary:** The image is a scatter plot labeled "Geospatial Wealth Map." The plot displays a distribution of colored dots, with the color representing different "price" values along a longitude and latitude coordinate system.

![](https://res.cloudinary.com/douppa0ns/image/upload/v1768385791/chatbot/files/img_p7_1_dakpg9.png)
...
**Summary:** The image displays six aerial views of properties with listed prices. The views show a variety of housing styles and landscapes, ranging from dense trees to suburban layouts.

![](https://res.cloudinary.com/douppa0ns/image/upload/v1768385801/chatbot/files/img_p8_1_atsvmz.png)
...
**Summary:** The image shows eight aerial views of residential areas overlaid with color gradients. Each image is labeled "RANDOM" and includes a price prediction in dollars.

![](https://res.cloudinary.com/douppa0ns/image/upload/v1768385810/chatbot/files/img_p9_1_ymjs5b.png)
...
The following table represents the grid of satellite images with Grad-CAM heatmaps shown in Figure 6. Each cell contains the label and predicted price for the corresponding image.

|       | Column 1              | Column 2              | Column 3              | Column 4              |
| ----- | --------------------- | --------------------- | --------------------- | --------------------- |
| Row 1 | RANDOM Pred: $923,852 | RANDOM Pred: $900,134 | RANDOM Pred: $906,567 | RANDOM Pred: $462,929 |
| Row 2 | RANDOM Pred: $725,780 | RANDOM Pred: $474,109 | RANDOM Pred: $888,414 | RANDOM Pred: $675,555 |
...
**Summary:** The image presents eight aerial views of residential areas, each overlaid with a colored map and a predicted value. The text below each image indicates if it is random, best, or worst.

![](https://res.cloudinary.com/douppa0ns/image/upload/v1768385768/chatbot/files/img_p10_1_xkxr6u.png)
...
**Summary:** The image displays four aerial views of residential areas, each overlaid with a colored heatmap and a text description, including the word "WORST" and a dollar amount. The heatmaps use a color gradient to highlight specific areas within the images.

![](https://res.cloudinary.com/douppa0ns/image/upload/v1768385775/chatbot/files/img_p10_2_tzoyy7.png)
...
graph LR
    subgraph Inputs
        A["Satellite Image<br/>(64×64×3)"]
        B["Tabular Features<br/>(20 features)"]
        C["Zipcode<br/>(categorical)"]
    end

A --> D["EfficientNet-B0"] --> E("1280")
    B --> F["MLP<br/>(128)"] --> G["MLP<br/>(64)"] --> H("64")
    C --> I["Embedding<br/>(dim=8)"] --> J("8")

E --> K["Concatenate<br/>(1352-dim)"]
    H --> K
    J --> K
...
graph LR
    subgraph Inputs
        SI[Satellite Image<br/>(64×64×3)]
        TF[Tabular Features<br/>(22 features)]
        ZC[Zipcode<br/>(categorical)]
    end

SI --> EN[EfficientNet-B0<br/>(frozen)]
    EN --> D1280[1280]

TF --> SKNN[Spatial KNN<br/>(neighbor avg)]
    SKNN --> D22KNN[22+KNN]

ZC --> EMB[Embedding<br/>(dim=8)]
    EMB --> D8[8]

D1280 --> FV[Feature Vector<br/>(1310-dim)]
    D22KNN --> FV
    D8 --> FV
...
style SI fill:#e1e5ff,stroke:#333,stroke-width:1px
    style TF fill:#e1e5ff,stroke:#333,stroke-width:1px
    style ZC fill:#e1e5ff,stroke:#333,stroke-width:1px
    style EN fill:#e1ffe1,stroke:#333,stroke-width:1px
    style SKNN fill:#e1f5ff,stroke:#333,stroke-width:1px
    style EMB fill:#e1ffe1,stroke:#333,stroke-width:1px
    style FV fill:#fffbe1,stroke:#333,stroke-width:1px
    style CB fill:#ffe1e1,stroke:#333,stroke-width:1px
    style PR fill:#fff0e1,stroke:#333,stroke-width:1px
```
...
| Source<br/>Total      | Dimensions<br/>1310 | Description<br/>Final input to CatBoost |
| --------------------- | ------------------- | --------------------------------------- |
...
* **$R^2$ (Coefficient of Determination)**: Measures the proportion of variance explained by the model. Higher is better (max = 1.0).
